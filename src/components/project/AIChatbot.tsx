...
interface AIChatbotProps {
  ...
  onUpdateSelectedNodeContent?: (nodeId: string, newContent: string) => void;
}
...
export const AIChatbot = forwardRef<AIChatbotRef, AIChatbotProps>(({ 
  projectId, 
  aiSettings, 
  selectedCanvasItem, 
  onUpdateSelectedNodeContent 
}, ref) => {
  ...
  // Helper function to check if user request is a modification request
  const isModificationRequest = (userInput: string): boolean => {
    const modificationKeywords = [
      'add', 'insert', 'update', 'change', 'modify', 'rewrite', 'append', 'prepend',
      'at the top', 'at the bottom', 'cta', 'call to action', 'improve', 'enhance',
      'make it', 'could you', 'can you add', 'please add', 'include', 'put in'
    ];
  ...
  const generateContextualWelcomeMessage = (userRequest?: string) => {
    ...
    // Add contextual instructions about node editing
    if (aiSettings.persona === 'default') {
      message += `\n\n**What would you like to create today?**

💡 **Pro tip:** Select any content node on the canvas and ask me to modify it! I can add CTAs, rewrite sections, or make any changes you need.`;
    }
  ...
  // Handle sending message with contextual node editing
    ...
    try {
      ...
      if (selectedCanvasItem && isModificationRequest(currentInput) && onUpdateSelectedNodeContent) {
        ...
        const confirmationMessage: ChatMessage = {
          id: `ai_${Date.now()}`,
          type: 'ai',
          content: generateModificationConfirmationMessage(selectedCanvasItem.title),
          timestamp: new Date(),
          suggestions: [
            'Make another change',
            'Create a variation',
        ...
        // Generate new content and add to canvas
  ...
  // Generate modification confirmation message
  const generateModificationConfirmationMessage = (nodeTitle: string) => {
    const baseMessage = `✅ **Content node updated successfully!**\n\n**"${nodeTitle}"** has been modified based on your request.`;
    
    switch (aiSettings.persona) {
      case 'seth_rogen':
        return `${baseMessage}\n\nNice! That looks way better now, dude. The changes really make it pop! Want me to tweak anything else?`;
      case 'alex_hormozi':
        return `${baseMessage}\n\nDone. The content is now optimized and ready to convert. Review the changes and deploy when ready.`;
      case 'gary_vaynerchuk':
        return `${baseMessage}\n\nBOOM! Those changes are FIRE! The content is now dialed in and ready to CRUSH IT! What's next?`;
      case 'oprah_winfrey':
        return `${baseMessage}\n\nBeautiful! The content now shines even brighter and will touch hearts in a more meaningful way. What other magic shall we create?`;
      default:
        return `${baseMessage}\n\nThe content has been updated according to your specifications. You can make additional changes or continue with other content.`;
    }
  };
      ...
      <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-50 to-purple-50">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                {selectedCanvasItem 
                  ? `Editing: ${selectedCanvasItem.title.slice(0, 20)}${selectedCanvasItem.title.length > 20 ? '...' : ''}`
          ...
          <div className="flex items-center gap-2">
            {/* Context Indicators */}
            {(aiSettings.brandVoiceId || aiSettings.audienceId || aiSettings.knowledgeIds.length > 0 || (aiSettings.persona && aiSettings.persona !== 'default') || selectedCanvasItem) && (
              <div className="flex items-center gap-1">
                ...
                {selectedCanvasItem && <div className="w-2 h-2 bg-red-500 rounded-full" title="Node Selected" />}
              </div>
            )}
            <Button
              variant="ghost"
      ...
      <div className="p-6 border-t border-gray-200 bg-gray-50">
        ...
        {selectedCanvasItem && (
          <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-blue-700">{selectedCanvasItem.title}</span>
            </div>
            <p className="text-xs text-blue-600 mt-1">
              Ask me to modify this content (e.g., "add a CTA at the bottom")
            </p>
          </div>
        )}
        ...
        <div className="flex gap-3">
          <input
            ...
            placeholder={selectedCanvasItem 
              ? "Ask me to modify the selected content..."
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {(aiSettings.brandVoiceId || aiSettings.audienceId || aiSettings.knowledgeIds.length > 0 || (aiSettings.persona && aiSettings.persona !== 'default') || selectedCanvasItem) && (
            <div className="flex items-center gap-1 text-xs text-indigo-600">
              <Sparkles size={12} />
              <span>{selectedCanvasItem ? 'Node Context' : 'AI Context'} Active</span>
            </div>
          )}
        </div>
      </div>
